...
export default function CookieConsent() {
  ...
  return (
    <div role="dialog" aria-modal="true" aria-label="Cookie consent" style={{ position: "fixed", bottom: 0, left: 0, right: 0, zIndex: 9999, backgroundColor: "#0C1421", borderTop: "1px solid rgba(255,255,255,0.12)", padding: "20px clamp(16px, 4vw, 40px)", boxShadow: "0 -4px 24px rgba(0,0,0,0.5)", animation: "sw-slide-up 0.45s cubic-bezier(0.22, 1, 0.36, 1) both" }}>
      ...
      <div style={{ maxWidth: "1280px", margin: "0 auto", display: "flex", flexWrap: "wrap", alignItems: "flex-start", gap: "16px", paddingRight: "32px" }}>
        <div style={{ flex: "1 1 480px", minWidth: 0 }}>
          <p style={{ fontFamily: "'Outfit', sans-serif", fontWeight: 700, fontSize: "16px", color: "#F5F5F0", marginBottom: "8px" }}>We respect your privacy</p>
          <p style={{ fontFamily: "'Inter', sans-serif", fontWeight: 400, fontSize: "14px", color: "#A8B2C7", lineHeight: 1.6, margin: 0 }}>
      ...
      <div style={{ maxWidth: "1280px", margin: "12px auto 0" }}>
        ...
        {customizeOpen && (
          <div style={{ marginTop: "16px", padding: "20px", backgroundColor: "#0A0E27", border: "1px solid rgba(255,255,255,0.12)", borderRadius: "16px", display: "flex", flexDirection: "column", gap: "16px" }}>
            {[
              ...
              <div key={item.key} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: "8px" }}>
                <div>
                  <p style={{ fontFamily: "'Inter', sans-serif", fontWeight: 600, fontSize: "14px", color: "#F5F5F0", margin: "0 0 2px" }}>{item.title}</p>
                  <p style={{ fontFamily: "'Inter', sans-serif", fontWeight: 400, fontSize: "13px", color: "#A8B2C7", margin: 0 }}>{item.desc}</p>
                </div>
                <button role="switch" aria-checked={item.checked} disabled={item.disabled} onClick={() => !item.disabled && item.key === "analytics" && setPrefs((p) => ({ ...p, analytics: !p.analytics })) || !item.disabled && item.key === "marketing" && setPrefs((p) => ({ ...p, marketing: !p.marketing }))} style={{ position: "relative", width: "48px", height: "26px", borderRadius: "13px", border: "none", cursor: item.disabled ? "not-allowed" : "pointer", backgroundColor: item.checked ? "#D4A017" : "#334155", transition: "background-color 0.2s ease", flexShrink: 0, opacity: item.disabled ? 0.6 : 1, padding: 0 }}>
