    ...
    <div role="dialog" aria-modal="true" aria-label="Cookie consent" style={{ position: "fixed", bottom: 0, left: 0, right: 0, zIndex: 9999, backgroundColor: "var(--sw-bg-secondary)", borderTop: "1px solid var(--sw-border)", padding: "20px clamp(16px, 4vw, 40px)", boxShadow: "0 -4px 24px var(--sw-overlay)", animation: "sw-slide-up 0.45s cubic-bezier(0.22, 1, 0.36, 1) both" }}>
      ...
      <div style={{ maxWidth: "1280px", margin: "0 auto", display: "flex", flexWrap: "wrap", alignItems: "flex-start", gap: "16px", paddingRight: "32px" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: "12px", flexShrink: 0, flexWrap: "wrap" }}>
          <button onClick={() => save({ analytics: false, marketing: false, functional: false })} className="hover:border-slate-400 transition-colors" style={{ fontFamily: "'Inter', sans-serif", fontWeight: 600, fontSize: "14px", color: "var(--sw-text)", backgroundColor: "transparent", border: "1px solid var(--sw-toggle-off)", borderRadius: "6px", padding: "10px 20px", cursor: "pointer", minHeight: "44px", whiteSpace: "nowrap" }}>Reject All</button>
      ...
      <div style={{ maxWidth: "1280px", margin: "12px auto 0" }}>
        ...
        {customizeOpen && (
          <div style={{ marginTop: "16px", padding: "20px", backgroundColor: "var(--sw-bg)", border: "1px solid var(--sw-border)", borderRadius: "16px", display: "flex", flexDirection: "column", gap: "16px" }}>
            {[
              ...
              <div key={item.key} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: "8px" }}>
                <div>
                  <p style={{ fontFamily: "'Inter', sans-serif", fontWeight: 600, fontSize: "14px", color: "var(--sw-text)", margin: "0 0 2px" }}>{item.title}</p>
                  <p style={{ fontFamily: "'Inter', sans-serif", fontWeight: 400, fontSize: "13px", color: "var(--sw-text-muted)", margin: 0 }}>{item.desc}</p>
                </div>
                <button role="switch" aria-checked={item.checked} disabled={item.disabled} onClick={() => !item.disabled && item.key === "analytics" && setPrefs((p) => ({ ...p, analytics: !p.analytics })) || !item.disabled && item.key === "marketing" && setPrefs((p) => ({ ...p, marketing: !p.marketing }))} style={{ position: "relative", width: "48px", height: "26px", borderRadius: "13px", border: "none", cursor: item.disabled ? "not-allowed" : "pointer", backgroundColor: item.checked ? "#D4A017" : "var(--sw-toggle-off)", transition: "background-color 0.2s ease", flexShrink: 0, opacity: item.disabled ? 0.6 : 1, padding: 0 }}>
                  <span style={{ position: "absolute", top: "3px", left: item.checked ? "25px" : "3px", width: "20px", height: "20px", borderRadius: "50%", backgroundColor: "var(--sw-toggle-knob)", transition: "left 0.2s ease", display: "block" }} />
                </button>
              </div>
            ))}
